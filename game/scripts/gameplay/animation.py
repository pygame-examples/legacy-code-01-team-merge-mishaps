...
class ANIMATIONMYWAY:
    def __init__(self, spritesheet_path: str, fps: int, single_frame_rect: pygame.Rect=(0,0,-1,-1), frame_count: int=-1,  loop_type="wrap", scale_factor: int=1, rotation: int=0):
        ...
        """
        self.frames = self._get_animation(spritesheet_path, single_frame_rect, frame_count, scale_factor, rotation)
    ...
    def _get_animation(self, spritesheet_path: str, frame_rect: pygame.Rect, frame_count: int, scale_factor: int, rotation: int):
        ...
        elif frame_rect == (0, 0, -1, -1):  # if no rect was given, calculate it using the number of frames
            frame_rect = pygame.Rect(0, 0, spritesheet.width, spritesheet.height//frame_count)

        frames = []

        for i in range(frame_count):
            frame = spritesheet.subsurface((frame_rect.x, frame_rect.height*i, frame_rect.width, frame_rect.height))
