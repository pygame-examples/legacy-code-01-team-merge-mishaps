...
class Door(PhysicsSprite):
    def __init__(self, data: SpriteInitData):
        ...
        self.segments = {  # the entire door will be drawn by segments
            "head": pygame.transform.scale_by(
                spritesheet.subsurface((0, 0, 32, 16)), scale_factor
            ),
            "middle": pygame.transform.scale_by(
                spritesheet.subsurface((0, 16, 32, 16)), scale_factor
            ),
            "base": pygame.transform.scale_by(
                spritesheet.subsurface((0, 32, 32, 16)), scale_factor
            ),
            "tip": pygame.transform.scale_by(
                spritesheet.subsurface((32, 0, 32, 16)), scale_factor
            ),
            "light-green": pygame.transform.scale_by(
                spritesheet.subsurface((32, 16, 32, 16)), scale_factor
            ),
            "light-red": pygame.transform.scale_by(
                spritesheet.subsurface((32, 32, 32, 16)), scale_factor
            ),
        }
    ...
    def draw(
        ...
        # draw the bar
        times_to_draw_middle = (
            ceil(self.current_height / self.segments["middle"].height) - 1
        )
        ...
        self.current_height = min(
            max(self.current_height, self.min_height), self.max_height
        )
